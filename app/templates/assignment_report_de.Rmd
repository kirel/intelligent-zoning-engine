...
tmp_shown = tmp_units %>%
  dplyr::filter(entity_id %in% names(color_vec))
...
table_data = data %>%
  ...
  dplyr::left_join(params$entities@data %>% dplyr::select(entity_id, capacity)) %>%
  dplyr::mutate(utilization = scales::percent(pop / capacity))
```
